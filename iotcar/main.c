...
void initial(void){
    ...
    socka=client_initial(sock_tcp,"192.168.0.105");
...
int main(int argc, char* argv[]){
	...
	while(1){
		...
		printf("L: %f\tR: %f\tmode: %c\n",wheelControl->left_duty,wheelControl->right_duty,mode);
   **/
	}
...
void *thread_tcp(void *arg){
   ...
   while(1){
    ...
    printf("\n current position is %c\n ", position);
	  ...
	  if((position=='a') ||(position=='b')|| (position=='c') || (position == 'A')|| (position=='B')|| (position == 'C')){
		  client_send_char(sock_tcp,socka,position);
		  mode=client_recv_char(sock_tcp,socka);
		  printf("change mode: %c!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n\n\n",mode);
      position=0;
	  }
